import { Box, Button, Grid, Paper, SelectChangeEvent, TextField, Typography } from "@mui/material";
...
export default function CreateTicketPage({categories, setCategories, editMode}: {categories: Category[], setCategories: (newCategories: Category[]) => void, editMode?: boolean }) {
    if (editMode) {
    ...
    const [assigneeIdsHasError, setAssigneeIdsHasError] = React.useState<number | undefined>(1);
    const [formError, setFormError] = useState('');

    const handleSubjectChange = (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    return (
        <Grid>
            ...
            >
                ...
                <TextField value={subject} onChange={handleSubjectChange} error={subjectError} helperText={subjectHelperText} margin="normal" id="outlined-basic" label="Subject" variant="outlined" />
                <Box sx={{ mt: 1, mb: 1  }}>
                    <Wysiwyg
                        value={description}
                ...
                <CategorySelect categories={categories} setCategories={setCategories} selectedIndex={categoryId} onChange={handleCategorySelectChange} />
                ...
                />
                {formError &&
                    <Typography align="center" component="h2" variant="h6" color="error" sx={{ mt: 2 }}>
                        ERROR: {formError}
                    </Typography>
                }
                <Button
